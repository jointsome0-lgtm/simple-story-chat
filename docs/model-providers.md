...
Добавлено 18 сентября 2026. Адаптер работает с любым API в формате OpenAI Chat Completions: `SIMPLE_CHAT_BASE_URL` задаёт версионный корень (`https://openrouter.ai/api/v1`, `https://api.openai.com/v1`), `SIMPLE_CHAT_API_KEY` и `SIMPLE_CHAT_MODEL` обязательны. Код общий с `llama-cpp` и находится в `local/llama.ts`; проверен только тестами с подставным `fetch`, живого вызова ещё не было.

Подключение предназначено только для синтетических проб. Размещённый провайдер может сохранять запросы и обучаться на них: бесплатные модели OpenRouter и бесплатная дневная квота OpenAI выдаются именно на таких условиях. Поэтому `loadConfig` отказывается запускать бота с этим провайдером, а `story:probe` читает `loadModelConfig` и работает. `memory:probe` идёт через очередь запущенного бота и с этим подключением пока не работает.
...
- `outputSchema` включает только режим `json_object` без схемы. Бесплатная Gemma 4 на OpenRouter не заявляет `structured_outputs`; результат проверяет приложение.
...
Добавлено 18 сентября 2026; живого прогона ещё не было. Цель — одно число для сравнения версий промптов и памяти на нескольких моделях сразу. Ключи размещённых API лежат в `.env.eval` (шаблон `.env.eval.example`); пробы запускаются в пустом каталоге, поэтому `.env` бота до них не доходит.
...
Добавлено 18 сентября 2026. Каждый запрос `openai-compatible` проходит через `local/budget.ts`. Счётчик лежит в `eval-usage.sqlite` в корне проекта, общий для параллельных проб; в нём день по UTC, имя канала, число запросов и токенов, без текста. `npm run eval -- usage` показывает расход за сегодня и действующие лимиты.
