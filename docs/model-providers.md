...
Three commands look after the tree. `eval gold-audit --judges <models> --scenarios <walk> --out <directory>` gives every judge the whole trunk at once, seed and all scenes, for contradictions between scenes that scene-by-scene reading let through and for the ambiguities the scenes themselves introduce (a new fact readable two ways divides a judge and a model continuing from the tree); the merged list lands in `<directory>/<walk>/issues.json` with the scene numbers. `eval gold-recheck --judges <models> --scenarios <walk> [--nodes g1,g2]` judges every trunk node again, fresh, and reports how many the judges agree to a second time: the gate's own noise, measured, before the tree is trusted. `eval gold-read --scenarios <walk> --nodes g1,g2` records that a person has read those scenes; until then the rendering marks each one as unread, because the council is made of models and the writers sit on it.
