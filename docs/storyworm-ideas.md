...
2026-09-18 · Opus 5 · прочитаны главная ветка storyworm и замороженная ветка `reference-001` (`048b166b`). Ссылки: `main:<путь>:<строка>` — главная ветка `/home/jo/work/story`, `ref:<путь>:<строка>` — `reference-001`, `commit:<sha>` — коммит storyworm. Пути даны от корня репозитория.
