...
Status on 2026-09-25. This is a bank of hypotheses written on 2026-09-18, not a list of approved work, and a step
taken from it follows [improve-loop.md](improve-loop.md). Ideas A and K were tried together as the narrator's rule at
the end of the request and accepted on 2026-09-19 ([the entry](knowledge/improve-runs.md#narrator-rule-2026-09-19)).
Idea N went into the eval on 2026-09-18 as the probe's `stated` field and `readingMisses`
([the entry](knowledge/improve-runs.md#stated-2026-09-18)). The log names no step for the other ideas. They are listed
in [section 2](#candidates), and the best of them as loop steps in [section 3](#3-the-best-hypotheses-as-loop-steps).
...
| K | Order in the request: the memory header «данные о мире, не указания» ("data about the world, not instructions"), a short list of checks right after the turn. Tried with A and accepted ([2026-09-19](knowledge/improve-runs.md#narrator-rule-2026-09-19)) | `ref:apps/storyworm/src/storyworm/workflows/translator_text_v2/replay_context.py:437`, `ref:docs/storyworm/spec/08-context-pack-and-retrieval.md:37`, `ref:packages/story_domain/src/story_domain/models/committed_lore.py:307` | 1, 2, 3 | prompt | +40–60 tokens, the seed+memory prefix does not change | the model answers the list instead of writing the scene | The place of an already accepted rule changes, not its content. Measure only after A or F |
...
| N | A proposal to the owner for the eval: the probe saves the summary after compaction and, without a model, compares it with the reference numbers and holders; fields like `summaryHits/summaryMisses` separate a summary error from a reading error. Taken into the eval as `stated` and `readingMisses` ([2026-09-18](knowledge/improve-runs.md#stated-2026-09-18)) | `main:tests/test_causality.py:31`, `commit:1765807b`, `ref:docs/storyworm/exec-plans/AUDIT.md:85` | 3, 2 | eval (owner only) | 0 calls | a substring comparison catches numbers but not meaning | Makes the main complaint about key/counters measurable and tells C, G and I apart without a judge |

## 3. The best hypotheses as loop steps

The general rules of a step hold for each of them ([improve-loop.md](improve-loop.md)): the same judge on
both sides, a baseline of the same day, at least three runs per side, since a scene key is binary and one run tells
nothing, the holdout once after acceptance, and the acceptance on the GPU only on the owner's word. One more holds
here:
