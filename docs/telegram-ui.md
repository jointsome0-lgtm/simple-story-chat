...
- **Story tree and scene log:** История → «🌳 Дерево истории» draws the story the way a commit graph with tags reads: a scene is a commit, and only the places that mean something are nodes — a memory compaction (🗜), a fork, a checkpoint the author saved (📍), the head of a branch (🌿, ✅ for the active one). A straight run between them is one line with its scene count and the world time of its last scene; the drawing is a `pre` block of at most 40 lines. Under it, and on the branch screen, «📜 Сцены ветки» lists every scene of one branch, newest first, 8 per page: number, world time, the names that point at the scene (🌿, 🗜, 📍, ⑂ where another branch leaves the line, ⚠️ for a truncated scene) and the first words of the author's input. Each scene opens its checkpoint preview, where «🌿 Продолжить отсюда» forks.
