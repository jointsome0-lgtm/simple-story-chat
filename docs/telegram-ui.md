...
- **Prompt under a picture:** every photo, the scene's own and every sample, gets a reply right after it: a rich message folded to one line, «🖼 Промпт: 243 токена, из них стиль 53 · 1 204 знака» ("Prompt: 243 tokens, 53 of them the style · 1,204 characters"). Opened, it shows the prompt the picture was drawn from in a code block, for the reader to read, copy and tune a style line against (`foldedPrompt` in `local/picture.ts`).
