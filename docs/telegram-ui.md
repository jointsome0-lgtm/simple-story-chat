...
written from. A frame takes the look, and a portrait the details ([Characters](#characters)). A sheet written before
2026-09-26 has no details and is not written again for them, so its portraits are drawn from the look; only the
rewrite of a sheet from before clothes left the look, which happens anyway, brings them. The frame of a scene is a
