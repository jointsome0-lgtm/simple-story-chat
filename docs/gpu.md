...
- A probe's own token count runs outside the slots too, but by a probe's rules: it runs only when the GPU is ready and there are no user jobs, waits in the probes' queue until then, keeps the GPU up from the moment the queue takes it until it has ended, and stops within a second once the GPU is no longer ready or a user job begins. Nobody waiting for a slot stops it: it takes none.
...
by request, each from the moment the bot's queue takes it until it ends: at most 10
minutes of waiting and a 90-second run. The timer does not run while any of this
work lasts and starts again when the last of it ends, so this work keeps the card up
for as long as it goes on: only the rental's own guard (see [Renting](#renting))
deletes the instance under it. `npm run eval` and the probes with `--direct` call
