...
**Prefer an offer that has direct ports.** An account key is necessary on a proxy-only machine and, on 2026-09-20, it was not sufficient: instance 51669851 refused it with `Permission denied (publickey)` for its whole life. The key was the account's one key (compared body to body), the API answered `SSH key already associated with instance`, the stored onstart still carried the line that writes it into `authorized_keys`, and `ssh -vvv` showed that single key offered and turned down. Detaching and reattaching the key, rebooting the container and twenty minutes of retries all changed nothing, and there is no way in from outside to repair it: `PUT /instances/command/{id}/` answers `Invalid command given` to anything outside its own small set, so the container cannot be told to install the key. The rental was deleted unused. A machine with direct ports keeps the container's own sshd reachable, which is the route that has worked; ask the search for `direct_port_count` and let the proxy-only offers go.
