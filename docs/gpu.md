...
Offers are sorted by `hour * 2.5 + download`, the cost of the session the instance is billed for, with the measured host first. With `--hours`, the session is those hours and 20 minutes more: the start before the guard's clock begins, and the destroy read back after it. `--qwen only` prices a picture machine by the Qwen files alone, which is what `SIMPLE_CHAT_IMAGE_QWEN=only` pulls. The dry run prints each offer's sum as `session`. Offers with fewer than two direct ports are dropped and the count of them is reported: an offer with no ports can only be reached through Vast's proxy. That rule has never yet excluded anything — every 5090 within this price has had ports — so treat it as a guard, not as an explanation of any failure.
...
by request, each from the moment the bot's queue takes it until it has ended locally.
The scheduler cancels a probe request that has waited 10 minutes or run 90 seconds
by its next tick, and the hold ends once the cancelled request has ended locally, so
there is no hard bound in seconds. The timer does not run while any of this work
lasts and starts again when the last of it ends, so this work keeps the card up for
as long as it goes on: only the rental's own guard (see [Renting](#renting)) deletes
the instance under it. `npm run eval` and the probes with `--direct` call
the model server directly: the bot does not see them (see
...
an agent's turn, but cancels a probe's request within a second and goes on once
that request has ended locally. The check runs every 10 seconds. Viewing the menu
and the status does not reset the timer. An ordinary message does not start a
stopped rental.
