...
У бота одна очередь вычислений в `local/scheduler.mjs`. Запросы пользователей обслуживаются по порядку. Фоновая проба начинает вычисление после 60 секунд без пользовательских запросов. Новый пользовательский запрос отменяет фоновое вычисление; проба повторяет только свой незавершённый шаг. Ответы пользователей автоматически не повторяются.
