...
`npm run eval` measures whether the system keeps the world continuous. After a recorded synthetic story, the model answers questions about memory and writes scenes for trap turns that push it to break what was established. A judge model grades the scenes by yes/no questions. `npm run eval -- walk` is the second measure: the model writes a story from a seed on its own, continuing or taking the author's interventions step by step, and a council of judge models reads every scene for contradictions with what came before. The design is in [docs/model-providers.md](docs/model-providers.md), the procedure for working on prompts and memory is in [docs/improve-loop.md](docs/improve-loop.md), and the history of changes with numbers is in [docs/improve-log.md](docs/improve-log.md).
