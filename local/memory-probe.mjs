...
import { createBackgroundClient } from './background.mjs';
import { compactBranch } from './generation.mjs';
import { Store } from './store.mjs';
import { contextParts, makeRequest } from './prompt.mjs';
import { addSeed, newStory, beginJob, commitTurn, active, context, history, emptyLibrary } from '../lib/library.js';
...
const scenario = frozen.report?.scenario;
if (!Object.hasOwn(checks, scenario)) throw new Error('Not a synthetic scenario');
const fixture = await import(`../examples/${scenario}-probe.mjs`);
...
const scenes = history(source.story, source.branch.head).filter(n => n.probeTurn <= fixture.turns.length);
...
const report = values.resume ? JSON.parse(readFileSync(join(directory, 'report.json'), 'utf8'))
...
const progress = data => console.log(JSON.stringify(data));
...
const provider = { async generate(request) {
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    const state = await client.check({ signal: deadline });
    ...
    try { return await client.generate(request, { signal: deadline }); }
      ...
      if (!['background_preempted', 'background_unavailable'].includes(error.code)) throw error;
      current.preemptions++;
      save();
      progress({ event: 'yielded', code: error.code });
...
try {
  ...
  for (const memoryMode of ['plain', 'sgr']) {
    ...
    store.mutate('synthetic', state => {
      ...
      else { for (const key of Object.keys(state)) delete state[key]; Object.assign(state, emptyLibrary());
    ...
    if (!Array.isArray(parsed.answers) || parsed.answers.length !== questions.length || new Set(parsed.answers.map(a => a.key)).size !== questions.length) {
      throw Object.assign(new Error(), { code: 'invalid_recall' });
    }
    current.answers = questions.map(([key, , expected]) => {
      const actual = parsed.answers.find(a => a.key === key)?.value;
  ...
  const code = deadline.aborted ? 'deadline' : /^[a-z_]{1,40}$/.test(error.code ?? '') ? error.code : 'probe_failed';
