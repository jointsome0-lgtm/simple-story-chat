...
const request = { system: 'synthetic system', messages: [{ role: 'user', content: 'synthetic private seed' }], maxOutputTokens: 256 };
const init = { type: 'system', subtype: 'init', tools: [], mcp_servers: [], model: 'test-model' };
const chunk = text => ({ type: 'stream_event', event: { delta: { type: 'text_delta', text } } });
...
const start = (id, usage) => ({ type: 'stream_event', event: { type: 'message_start', message: { id, usage } } });
const delta = usage => ({ type: 'stream_event', event: { type: 'message_delta', usage } });
...
test('large UTF-8 input is allowed when its observed token count fits, and fails closed without usage', async t => {
  const large = { ...request, messages: [{ role: 'user', content: 'я'.repeat(40000) }] };
