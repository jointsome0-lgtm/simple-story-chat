...
import { render, renderContext, scenePrefix, sceneKeyboard } from './ui.mjs';
...
function node(id, parent, time, body, input = 'Ввод') {
...
function checkPayload(message, route) {
...
test('empty library guides to seed creation', () => {
  const state = { version: 1, seq: 0, seeds: {}, stories: {}, active: null, job: null, ui: null, seen: [] };
...
const current = overrides => stats({ scope: 'current', label: 'Начало', checkpointId: null, ...overrides });
...
test('scene prefix is a minimal approximate percentage', () => {
  ...
  for (const unknown of [null, undefined, {}, current({ request: null }), current({ request: { estimatedTokens: null } }), current({ limitTokens: null }), current({ limitTokens: 0 })]) {
...
function gpuInfo(overrides = {}) {
  return { status: 'ready', activeJobs: 0, idleMinutes: 15, idleRemainingSeconds: 600, canStart: false, canPause: true, ...overrides };
}
...
test('gpu status texts are honest about billing, waiting and unknown states', () => {
  const state = fixture();
  const text = (overrides, details = {}) => render(state, 'model', { modelInfo: modelInfo(), gpuInfo: gpuInfo(overrides), ...details }).text;
  ...
  const busy = render(state, 'model', { modelInfo: modelInfo(), gpuInfo: { ...gpuInfo({ activeJobs: 3 }), users: ['Алиса'], storyTitle: 'Секрет' } }).text;
...
test('gpu info is hidden for the Claude provider and when absent', () => {
  ...
  for (const gpu of [undefined, null, 'ready']) {
    const message = render(state, 'model', { modelInfo: modelInfo(), gpuInfo: gpu });
...
test('home shows a compact gpu line without controls', () => {
  const state = fixture();
  const home = overrides => render(state, 'home', { modelInfo: modelInfo(), gpuInfo: gpuInfo(overrides) });
...
test('compact now is offered only for the idle current branch', () => {
  ...
  for (const kind of [undefined, 'scene', 'compact']) {
    state.job = { id: 'j12', kind, storyId: 'h2', branchId: 'b3' };
...
test('busy labels distinguish compaction from scene generation', () => {
  const state = fixture();
  state.job = { id: 'j12', kind: 'compact', storyId: 'h2', branchId: 'b3' };
  const home = render(state);
  ...
  state.job = { id: 'j13', kind: 'scene', storyId: 'h2', branchId: 'b3' };
...
test('context routes use only matching stats', () => {
  ...
  state.job = { id: 'j12', storyId: 'h2', branchId: 'b3' };
