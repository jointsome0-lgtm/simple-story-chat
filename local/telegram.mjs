...
export class TelegramError extends Error {
  constructor(code, retryAfter) { super(`telegram_${code}`); this.code = code; this.retryAfter = retryAfter; }
}
...
export function createChat(api, chatId) {
  return {
    send: screen => api('sendMessage', { chat_id: chatId, ...screen }),
    edit: (messageId, screen) => api('editMessageText', { chat_id: chatId, message_id: messageId, ...screen }),
    async final(text, replyMarkup) {
      return api('sendRichMessage', { chat_id: chatId, rich_message: { markdown: text },
        ...(replyMarkup ? { reply_markup: replyMarkup } : {}) });
    },
    preview(jobId, prefix = '') {
      let text = prefix;
      let next = 0;
      return async delta => {
        text += delta;
        ...
        } catch (error) { next = Date.now() + (error.retryAfter || 5) * 1000; }
      };
    },
  };
}
