...
This directory implements the owner-approved local fallback while Telegram Serverless is waitlisted. It runs on Node 24, uses relative ESM imports and node:sqlite, and is not deployed by tgcloud. The root scaffold's V8/SDK-only restrictions describe the cloud files, not this directory.
