...
const chunk = (delta, finish = null) => ({ model: config.model, choices: [{ index: 0, delta, finish_reason: finish }] });
const usage = { prompt_tokens: 120, completion_tokens: 8, prompt_tokens_details: { cached_tokens: 100 } };
const json = value => new Response(JSON.stringify(value), { headers: { 'Content-Type': 'application/json' } });
function stream(items, { split = false, done = true } = {}) {
...
function fixture(respond = () => stream([chunk({ content: 'Готово.' }, 'stop'), { choices: [], usage }])) {
  const calls = [];
...
test('schema-constrained requests keep the same schema for counting and generation; scenes stay free-form', async () => {
  ...
  const r = { ...request(), outputSchema, purpose: 'memory' };
...
test('HTTP failure returns a safe code without retries or raw error disclosure', async () => {
  for (const [status, code] of [[401, 'unauthorized'], [429, 'rate_limited'], [503, 'model_unavailable'], [404, 'unsupported_server']]) {
    let calls = 0;
...
test('generation HTTP failures preserve their stage and status without reading the error body', async () => {
  ...
  await assert.rejects(f.provider.generate(request()), error => {
...
test('cancellation and timeout close the in-flight request', async () => {
  const waitForAbort = async (_, options) => new Promise((resolve, reject) => {
