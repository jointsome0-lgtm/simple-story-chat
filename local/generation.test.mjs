...
import { ModelError } from './claude.mjs';
import { makeRequest } from './prompt.mjs';
import { requestBudget } from './context.mjs';
import { addSeed, newStory, beginJob, commitTurn, context, fork, deleteSeed } from '../lib/library.js';
...
function fixture(t, count = 7) {
  ...
  const config = { contextTokens: 65536, maxOutputTokens: 4096, model: 'test-model', keepScenes: 4, repairCoverage: true };
  ...
  const provider = { async generate(request, controls) {
    ...
    return { text: '2026-08-02 20:00\n\nНовая сцена.', finishReason: 'stop', usage: { inputTokens: 1000, outputTokens: 20 } };
  } };
...
test('compaction at the threshold keeps four recent scenes and both reversible checkpoints', async t => {
  ...
  const summaryData = JSON.parse(f.calls[0].request.messages[0].content);
  assert.equal(summaryData.newScenes.length, 3);
  assert.deepEqual(f.calls[0].request.outputSchema.properties.facts.items.properties.source.items.enum,
...
test('one coverage supplement targets only missed scenes and commits complete memory atomically', async t => {
  ...
  f.provider.generate = async request => {
    ...
    if (requests.length === 1) {
      ...
      assert.deepEqual(request.outputSchema.properties.facts.items.properties.source.items.enum, [requests[0][1]], 'context-only scenes cannot be cited');
      assert.equal(request.outputSchema.properties.facts.maxItems, 198);
    }
...
test('a still-incomplete or oversized supplement fails without partial memory or further retries', async t => {
  for (const growth of [false, true]) {
    ...
    f.provider.generate = async request => {
      calls++;
      const data = JSON.parse(f.summary(request).text);
...
test('coverage repair is opt-in; diagnostics alone preserve the single-call failure', async t => {
  ...
  f.provider.generate = async request => {
    calls++;
    const data = JSON.parse(f.summary(request).text); data.facts.pop();
    return { text: JSON.stringify(data), finishReason: 'stop' };
  };
  await assert.rejects(compactBranch({ store: f.store, userId: '1', jobId: f.job.id,
