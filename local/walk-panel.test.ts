...
test('contradictions decide the verdict, an empty inconsistent verdict abstains, a fenced reply is read', () => {
  // The judge of a scene sees the seed, every earlier step and the new scene, and answers by schema.
  ...
  // Its reply: evidence decides, and a malformed entry is dropped, so it cannot count as evidence.
  const c = { now: 'a', before: 'b', where: 'сцена 1', kind: 'number' };
  const replies: [string, string, string, number][] = [
    ['consistent, nothing listed', '{"verdict":"consistent","contradictions":[]}', 'consistent', 0],
    ['consistent, a contradiction listed', JSON.stringify({ verdict: 'consistent', contradictions: [c] }), 'inconsistent', 1],
    ['inconsistent, nothing listed', '{"verdict":"inconsistent","contradictions":[]}', 'error', 0],
    ['fenced', '```json\n{"verdict":"inconsistent","contradictions":[' + JSON.stringify(c) + ']}\n```', 'inconsistent', 1],
    ['inconsistent, only a malformed entry', JSON.stringify({ verdict: 'inconsistent', contradictions: [{ now: 'a' }] }), 'error', 0],
  ];
  for (const [label, reply, verdict, listed] of replies) {
    const parsed = parseVerdict(reply);
    assert.deepEqual([parsed.verdict, parsed.contradictions.length], [verdict, listed], label);
  }
  assert.throws(() => parseVerdict('{"verdict":"maybe","contradictions":[]}'), { code: 'invalid_verdict' });
  // Compaction after scene 7 and every fourth scene after it; a judge label becomes a file name.
...
test('the panel decides by majority; a tie is split, an error abstains, no vote is unjudged', () => {
  ...
  // The council numbers every listed contradiction per scene, asks about each, and lets the majority of checks decide.
  ...
  const cast = council(3, byJudge, { a: [check(1, true), check(2, false), check(3, true)], b: [check(1, true), check(2, false), check(3, false)], d: [check(1, false), check(2, false)] });
  assert.deepEqual(cast[1], { turn: 2, findings: 3, confirmed: 1, refuted: 1, disputed: 1, verdict: 'inconsistent' });
  assert.deepEqual(cast[0], { turn: 1, findings: 0, confirmed: 0, refuted: 0, disputed: 0, verdict: 'consistent' });
  assert.equal(cast[2].verdict, 'unjudged');
...
test('the story audit puts the seed and every scene in one request and keeps only issues that name a scene', () => {
  // The seed audit asks about the seed alone, keeps a contradiction or an ambiguity with its quote and note, and a reply
  // without a list of issues is no clean audit.
  assert.ok(seedAuditRequest('Сид.\n2026-01-01 10:00\nМир.').messages[0].content.startsWith('СИД:\nСид.'));
