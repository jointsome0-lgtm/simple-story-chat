...
export { ModelError } from './model-error.mjs';
...
function inputTokens(usage) {
  ...
  return parts.some(n => tokenCount(n) === null) ? null : parts.reduce((a, b) => a + b, 0);
}
...
export function createClaude(config, { launch = spawn } = {}) {
  return {
    async generate(request, { onText = async () => {}, signal, inputLimitTokens } = {}) {
      ...
      const env = { ...process.env, CLAUDE_CODE_MAX_OUTPUT_TOKENS: String(request.maxOutputTokens),
      ...
      try {
        ...
        if (measured?.inputTokens > inputLimit) throw new ModelError('context_limit');
