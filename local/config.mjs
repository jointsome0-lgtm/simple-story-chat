...
function environment(directory, inherited) {
  let file = {};
...
export function modelBaseUrl(value) {
  let url;
  try { url = new URL(value); } catch { throw new Error('Set SIMPLE_CHAT_BASE_URL to the llama.cpp server root'); }
...
function modelConfig(env) {
  ...
  if (!['claude-code', 'llama-cpp'].includes(provider)) throw new Error('Unsupported SIMPLE_CHAT_PROVIDER');
  ...
  if (!['plain', 'sgr'].includes(memoryMode)) throw new Error('Invalid SIMPLE_CHAT_MEMORY_MODE');
...
export function loadModelConfig(directory = process.cwd(), inherited = process.env) {
  return modelConfig(environment(directory, inherited));
}

export function gpuConfig(env, provider) {
...
export function loadConfig(directory = process.cwd(), inherited = process.env) {
