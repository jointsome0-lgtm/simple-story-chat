...
import { emptyLibrary, addSeed, newStory, beginJob, commitTurn, commitMemory, saveCheckpoint, fork } from '../lib/library.js';
import { contextStats, requestBudget, requestStamp, CONTINUE } from './context.mjs';
...
function fixture() {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    story.nodes[ref.nodeId].usage = { inputTokens: i * 100, outputTokens: i * 30 };
...
test('prefix includes only the checkpoint memory chain and separates unsummarized scenes', () => {
  ...
  commitMemory(state, job.id, turns.slice(0, 2).map(t => t.nodeId), { facts: [{ text: 'Память о двух сценах.' }] });
  ...
  commitMemory(state, job.id, [turns[2].nodeId], { facts: [{ text: 'Будущий инкремент не относится к старой точке.' }] });
...
test('input estimates use measured input only for the same model, rules and memory', () => {
  ...
  commitMemory(state, job.id, [Object.keys(story.nodes)[0]], { facts: [{ text: 'Сжатая сцена.' }] });
