...
import { parseMemory, summaryRequest } from './memory.mjs';
import { Store } from './store.mjs';
import { compactBranch } from './generation.mjs';
import { makeRequest } from './prompt.mjs';
import { addSeed, newStory, beginJob, commitTurn, active, context, jobTarget } from '../lib/library.js';
...
const result = data => ({ text: JSON.stringify(data), finishReason: 'stop' });
test('SGR keeps cancelled quantities, resolves sources mechanically and stores evidence separately', () => {
  ...
  assert.equal(parsed.sgr.evidence[2].quote, 'Он сыграл 4.Nf3.');
});
...
test('three SGR increments preserve checkpoints, exclude audit quotes from prompts and roll back invalid memory', async t => {
  ...
  const config = { memoryMode: 'sgr', maxOutputTokens: 4096, contextTokens: 65536, keepScenes: 4 };
  const checkpoints = [];
  for (const count of [7, 4, 4]) {
    ...
    const provider = { async generate(req) {
      assert.equal(req.maxOutputTokens, 8192);
      const data = JSON.parse(req.messages[0].content);
