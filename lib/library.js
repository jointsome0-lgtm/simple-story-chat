...
export function newStory(state, seedId) {
    const seed = state.seeds[seedId];
    if (!seed)
        throw new UserError('Сид уже удалён.');
    const story = { id: id(state, 'h'), seedId, title: seed.title, branches: {}, checkpoints: {}, nodes: {}, memories: {} };
    const branch = { id: id(state, 'b'), name: 'Начало', head: null, memory: null };
...
export function active(state) {
    ...
    if (!branch)
        throw new UserError('Выбери ветку истории через /seeds.');
...
export function fork(state, storyId, checkpointId) {
    const story = state.stories[storyId];
    const cp = story?.checkpoints[checkpointId];
    if (!cp)
        throw new UserError('Чекпоинт уже удалён.');
...
export function beginJob(state, input, now) {
    if (state.job)
        throw new UserError('Продолжение уже готовится. /cancel отменит его, если запрос завис.');
...
export function commitMemory(state, jobId, covered, delta) {
...
export function commitTurn(state, jobId, text, truncated = false) {
    ...
    if (!validTime(time))
        throw new UserError('Модель не указала корректные дату и время. Ответ не записан; отправь продолжение ещё раз.');
...
export function deleteBranch(state, storyId, branchId) {
    ...
    if (!story?.branches[branchId])
        throw new UserError('Ветка уже удалена.');
